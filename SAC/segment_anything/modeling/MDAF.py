...
class MDAF(nn.Module):
    def __init__(self, dim, num_heads, LayerNorm_type, ):
        ...
        self.conv1 = nn.Conv2d(256*2,256,1,1,0)

        self.conv3 = nn.Conv2d(256*2,256,3,1,1)
        self.conv = nn.Conv2d(768,384,1,1,0)
    ...
    def forward(self, x1,x2):
        ...
        v1 = rearrange(out1*cou, 'b (head c) h w -> b head h (w c)', head=self.num_heads)
        k2 = rearrange(out2, 'b (head c) h w -> b head w (h c)', head=self.num_heads)
        v2 = rearrange(out2*cou, 'b (head c) h w -> b head w (h c)', head=self.num_heads)
        ...
        out3 = self.bn(out3)
        out3sof= self.softmax(out3)

        out4 = self.project_out(out4)
        out4 = self.bn(out4)
        out4sof = self.softmax(out4)
        out3 = x1+out3sof
        out4 = x2+out4sof
